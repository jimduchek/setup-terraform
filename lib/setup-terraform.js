...
async function run () {
  try {
    ...
    const cache = core.getInput('tool_cache') === 'true';
    ...
    // Check tool-cache for requested version
    if (cache) {
      pathToCLI = await tc.find('terraform', version, `${platform}-${arch}`);
    }
    
    if (!pathToCLI) {
    // Download requested version
      pathToCLI = await downloadCLI(build.url);
      ...
      // Add to tool-cache    
      if (cache) {
        tc.cacheDir(pathToCLI, 'terraform', version, `${platform}-${arch}`);
      }
    }
